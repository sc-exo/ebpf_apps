			       ...
			       void *buf, size_t buf_size)
{
	...
	int ret;
	// int share_fd;
	// share_fd = shm_open("my_shm", O_CREAT | O_RDWR, 0666);
	// ftruncate(share_fd, 1024*1024*10);
...
static int __io_uring_queue_init_params_share(unsigned entries, struct io_uring *ring,
					struct io_uring_params *p, void *buf,
					size_t buf_size)
{
	int fd, ret = 0;
...
int io_uring_queue_init_mem_share(unsigned entries, struct io_uring *ring,
			    struct io_uring_params *p,
			    void *buf, size_t buf_size)
{
	/* should already be set... */
	p->flags |= IORING_SETUP_NO_MMAP;
	return __io_uring_queue_init_params_share(entries, ring, p, buf, buf_size);
}
