...
int __sys_io_uring_enter2_bpf(unsigned int fd, unsigned int to_submit,
...
int __sys_io_uring_enter_bpf(unsigned int fd, unsigned int to_submit,
				       unsigned int min_complete,
				       unsigned int flags, sigset_t *sig)
{
	return __sys_io_uring_enter2_bpf(fd, to_submit, min_complete, flags, sig,
				     _NSIG / 8);
}

void io_uring_cq_advance_bpf(struct io_uring *ring, unsigned nr)
{
	if (nr) {
...
int __io_uring_peek_cqe_bpf(struct io_uring *ring,
				      ...
				      unsigned *nr_available)
{
	...
	do {
		...
		if (!(ring->features & IORING_FEAT_EXT_ARG) &&
			...
			if (cqe->res < 0)
				err = cqe->res;
			io_uring_cq_advance_bpf(ring, 1);
			if (!err)
				continue;
			cqe = NULL;
		}
...
bool sq_ring_needs_enter_bpf(struct io_uring *ring,
				       unsigned submit,
				       unsigned *flags)
{
	if (!submit)
...
bool cq_ring_needs_flush_bpf(struct io_uring *ring)
{
	return IO_URING_READ_ONCE(*ring->sq.kflags) &
				 (IORING_SQ_CQ_OVERFLOW | IORING_SQ_TASKRUN);
}
...
bool cq_ring_needs_enter_bpf(struct io_uring *ring)
{
	return (ring->flags & IORING_SETUP_IOPOLL) || cq_ring_needs_flush_bpf(ring);
}
...
int _io_uring_get_cqe_bpf(struct io_uring *ring,
			     struct io_uring_cqe **cqe_ptr,
			     struct get_data *data)
{
	struct io_uring_cqe *cqe = NULL;
	...
	do {
		...
		ret = __io_uring_peek_cqe_bpf(ring, &cqe, &nr_available);
		if (ret) {
			if (!err)
				err = ret;
			break;
...
int __io_uring_get_cqe_bpf(struct io_uring *ring, struct io_uring_cqe **cqe_ptr,
		       unsigned submit, unsigned wait_nr, sigset_t *sigmask)
{
	struct get_data data = {
		.submit		= submit,
	...
	return _io_uring_get_cqe_bpf(ring, cqe_ptr, &data);
}
...
unsigned __io_uring_flush_sq_bpf(struct io_uring *ring)
{
	struct io_uring_sq *sq = &ring->sq;
	unsigned tail = sq->sqe_tail;
...
int __io_uring_submit_bpf(struct io_uring *ring, unsigned submitted,
			     unsigned wait_nr, bool getevents)
{
	bool cq_needs_enter = getevents || wait_nr || cq_ring_needs_enter_bpf(ring);
	unsigned flags;
	int ret;

	flags = 0;
	if (sq_ring_needs_enter_bpf(ring, submitted, &flags) || cq_needs_enter) {
		if (cq_needs_enter)
			flags |= IORING_ENTER_GETEVENTS;
		if (ring->int_flags & INT_FLAG_REG_RING)
			flags |= IORING_ENTER_REGISTERED_RING;

		ret = __sys_io_uring_enter_bpf(ring->enter_ring_fd, submitted,
					   wait_nr, flags, NULL);
	} else
		ret = submitted;
...
int __io_uring_submit_and_wait_bpf(struct io_uring *ring, unsigned wait_nr)
{
	return __io_uring_submit_bpf(ring, __io_uring_flush_sq_bpf(ring), wait_nr, false);
}
...
int io_uring_submit_bpf(struct io_uring *ring)
{
	return __io_uring_submit_and_wait_bpf(ring, 0);
}
...
int io_uring_wait_cqe_nr_bpf(struct io_uring *ring,
				      struct io_uring_cqe **cqe_ptr,
				      unsigned wait_nr)
{
	return __io_uring_get_cqe_bpf(ring, cqe_ptr, 0, wait_nr, NULL);
}

int io_uring_wait_cqe_bpf(struct io_uring *ring,
				    struct io_uring_cqe **cqe_ptr)
{
	if (!__io_uring_peek_cqe_bpf(ring, cqe_ptr, NULL) && *cqe_ptr)
		return 0;

	return io_uring_wait_cqe_nr_bpf(ring, cqe_ptr, 1);
}
